...
"""  
Coa seguinte orde configuramos o tipo de teclado que se presenta
na pantalla cando queremos introducir texto
"""
#'dock'            activa o teclado na parte inferior
#systemandmulti   activa o teclado nunha venta flotante que podemos mover
Config.set('kivy', 'keyboard_mode', 'systemanddock')

#importamos todas os widgets necesarios de kivy para a nosa inferface grafica
...
#  importamos as librerias de python necesarias.
import os
import re
...
"""
 importamos a libraria linphone que permite a comunicacion SIP empregada coa
 centralita PBX. Neste proxecto empregase para a comunicacion entre a panrtalla
 de control, o videoporteiro e os altofalantes coa camara integrada
 

"""
import linphone 
#  Libraria que implementa o reloxio dixital
from digitalclock import DigitalClock
#  Libraria que implementa o control de volume personalizado
from volumen_rect import VolumenRect
#  Libraria que implementa os diferentes tipos de botons persoalizados do proxecto
from botones import BotonColor,BotonI,BotonLed,IconBoton,IconButton,BotonLedToggle

#Varibles globais onde definimos as ips, portos e extensións dos diferentes dispositivos conectados
#Este tipo de variables poden ser consultadas e modificadas dende calquera clase ou metodo do programa

# Extensións dos diferentes departamentos do centro
EXT_USER1=6010
EXT_USER2=6013
EXT_DIRECCION=6002
EXT_XEFATURA=6003
...
EXT_VIDEOPORTERO=6000

"""
 Extensión da pantalla de control onde se instale o programa, será diferente para 
 cada terminal. As extensions deben ser do tipo numerico e os contrasinais strings
 Para a conversion dun dato numerico a string en python, empregase a funcion 
 str(dato)
"""
# Numero da extension coa que se rexistrara na centralita PBX
MIEXTENSION=config.get('terminal', 'extension')#6005
# Contrasinal para o aceso a PBX
MIEXTENSIONP=str(config.get('terminal', 'clave'))#'p6005'
# Ip da centralita PBx, por defecto empregaremos 192.168.0.150
IPCENTRALITA=str(config.get('terminal', 'ipbx')) 
# Rutas onde debe buscar as imaxes e sons do proxecto
PATH='./imaxes/'
PATHSOUNDS='./sons/'


MIIP=str(config.get('sistema', 'ip'))
"""
Crease as direccions dos diferentes terminais para a centralita PBX
Ten o seguinte formato do tipo string 'sip:NUMERO_DE_EXTENSION@IP_DA_CENTRALITA_PBX'
Para concatenar strings e numeros empregamos a funcion str unidos polo signo +
"""
...
#colocamos as direccions, na variable whitelist, das que a clase Phone aceptara chamadas.
#O resto seran rexeitadas
#A clase Phone verase mais adiante
whitelist=[ENTRADA_PRINCIPAL] 

#VARIABLES GLOBAIS REFERIDAS O ESTADO DAS CHAMADAS FEITAS A TRAVES DA CENTRALITA
llamada_entrante=''
llamada_in=False
llamada_saliente=''
...
#O volumen do sistema e o do micrófono deben ser do tipo float
...
""" 
#Esta clase serve para enviar comandos de control via ethernet, para activar sons
nos altofalantes, abrir a porta ou a luz do videoporteiro.
Empregase un socket UDP en modo cliente que se conecta os servidores(altofalantes,videoporteiro)
os que se quere enviar o comando de control.

"""

class EnviaComando():
    
    """Co metodo __init__ faise a inicializacion de todas as variables
    cada vez que se crea un obxeto desta clase.
     Python, dentro dos métodos definidos dunha clase, establece que o primeiro 
     parámetro definido nun método recibirá o obxecto co cal se invoca o devandito método. 
     Este parámetro (que se adoita chamar  self aínda que se pode usar calquera nome de 
     variable ) é usado dentro da implementación do método para modificar o contido ou 
     atributos do devandito obxecto como desexes.
    ...
    def __init__(self,ip,puerto):
        ...
        self.ip=ip
        self.puerto=puerto
        #crease un socket do tipo UDP (SOCK_DGRAM)
        self.client_socket =  socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
           
    def envia(self,comando):    
       
        direccion=(self.ip,self.puerto)
        # Empregase try,except para evitar que o programa xere un erro e pare no caso de que
        #o servidor non este activo
        try:
            self.client_socket.sendto(comando ,direccion)
        except:
            #poderiase completar o programa xerando unha mensaxe de aviso de erro na conexion
            return False
...
"""Clase DiplayCctv.
Esta clase lee un stream procedente dunha url en formato jpeg e transformalo
nun obxeto da imaxe en memoria de kivy para poder traballar con el
e visualizalo na pantalla.
Dispon de varios metodos, init, start, stop, read_stream, etc, todos eles
para controlar a visualizacion do video das camaras cctv integradas nos 
altofalantes. Este tipo de obxectos gráficos tamén se coñecen co nome de widgets
...
class DisplayCctv(Image): #SEVER
    ...
    """Creanse obxetos da clase StringProperty, unha clase especial de kivy que non
    ten que ver coas propiedades de python.
    Este tipo de propiedades en kivy permiten xerar eventos cada vez que se modifica o seu valor
    Debese facer coa palabra reservada on_nome_da_propiedade
    """
    #direccion dunha camara cctv
    url = StringProperty()  
    #hora actual que aparece no video da camara
    hora=StringProperty()
  
    def __init__(self, **kwargs):
        
        #con super herdamos da clase Image todos os atributos de esta
        super( DisplayCctv, self).__init__(**kwargs)   
        self._image_buffer = None
        self.cctv_no=Image( source='./imaxes/novideo600.png') 
        
    # Método no que se crea un novo fío que lee o strem dado por url.
    #Cada cámara existente crea un novo fío que se executa en paralelo
    def start(self):
    ...
    #Activase a variable quit que permite sair do bucle de lectura do video en streaming
    def stop(self):
        self.quit = True
    ...
    #Método para ler o stream de video
    def read_stream(self):
        #Inténtase abrir o stream da direccion dada por url
        try:
        ...
        while  not self.quit:    
            ...
            #caracteres que indican o inicio dun novo frame jpeg
            a = bytes.find('\xff\xd8')
            #caracteres que indican o fin dun novo frame jpeg
            b = bytes.find('\xff\xd9')
            #comprobase que se capturaron os dous indicadores, eliminanse e xa se obtén
            #o frame jpeg completo
            if a != -1 and b != -1: 
                jpg = bytes[a:b + 2] 
                bytes = bytes[b + 2:]
                #comvertimos o frame nun obxeto similar a un ficheiro, pero na memoria
                #( en python obxecto é io.BytesIO)
                data = io.BytesIO(jpg) 
                #creamos unha imaxe cos datos do frame almacenado na memoria
    ...
    def update_image(self, *args):
        ...
        #actualizase a hora ao mesmo tempo que a imaxe, en caso contrario a  fecha 
        #e hora se  sobreimpresiona so  unha vez
        
        # con isto  sacanse os tres derradeiros caracteres do string
        self.hora= str(datetime.datetime.now())[:-3] 
        
        self._image_buffer = None        
        if im is not None:            
            #asignamos a imaxe de kivy a textura e o tamaño do frame recibido
            self.texture = im.texture
            self.texture_size = im.texture.size
          

 ########### fin da clase DisplayCctv    
 
"""
Clase CamaraCctv, hereda das clases ButtonBehavior e AnchorLayout de kivy
Esta clase permite ver unha cámara coa hora sobreimpresionada mais o
nome asignado a esta.
Nesta clase emprégase python mais a linguaxe propia de kivy "kv"
Python emprégase para o control e kv para os elementos gráficos que 
se queren presentar na pantalla e a súa localización

""" 
class CamaraCctv(ButtonBehavior,AnchorLayout):
    #nome da camara do tipo StringProperty
    nombre=StringProperty()
    #ora en tempo real
    h=StringProperty()
    # dirección da cámara
    url = StringProperty()
    #Metódo que xera un evento cada vez que prememos na imaxe da cámara
    def on_press(self, *args):
        pass    
    #Método que inicia a captura dos frames da camara dada
    #Para o acceso os wigets
    def start(self):
        self.ids.camara.start()
    #Método que para a captura dos frames da camara dada
    def stop(self):
        self.ids.camara.stop() 
    #Método que actualiza a hora sobreimpresa no frame
...
class Cctv_screen(Screen):
    ...
    """ creamos un dicionario chamado  camara onde almacenamos a url e o nome 
        da  camara que queiramos poner a pantalla
        completa cando pulsemos nunha delas     
    """
